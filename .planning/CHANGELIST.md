...
| Phase 7 | v2.5 | Native-AI upload bundle + audit log (UAT-6.1) | 1 NEW | — (UNBLOCKED under UAT-6.1; see Phase 7 mini-table for revised scope) |
...
**OPEN-01 superseded by UAT-6.1 (2026-05-10).** The historic D-37 contingent fallback ("if OPEN-01 unresolved at v2.5 kickoff, slide P7 → v2.6 / P8+P9 → v2.7") is OBSOLETE — UAT-6.1 removes API-based native-AI ingestion from scope entirely (Q01/Q02/Q03 withdrawn; status `closed` under UAT-6.1). Phase 7 ships in v2.5 alongside Phase 6 with simplified scope (paste bundle + upload audit log; no API ingestion). Milestone bundling reverts to original CHANGELIST.md sequence — v2.5 = P6+P7; v2.6 = P8+P9.
...
| Attribute | Detail |
|---|---|
| Deliverables | (a) `dydx-delivery/references/{safety-rules.md, stage-numbering.md, frontmatter-scheme.md, glossary.md}` canonical references; (b) existing 7 skills updated to point at canonical references (collapses 4 hard-rules duplicates per AUDIT-05.1; fixes sandbox-block bug; normalises `based_on_*` field names); (c) file renumbering 00→02 / 01→03 / 02→04 / 03→05 / 04→07a per FOUND-03; (d) plugin manifest `2.0.0` synced across `plugin.json` + `marketplace.json` metadata + `plugins[0]`; (e) owner-email is INTENTIONAL per UAT-3.1 reclassification (2026-05-10) — no change; MIN-6 cross-ref closed/withdrawn; (f) `LICENSE` file added (content TBD per OPEN-Q23); (g) empty `commands/`, `agents/`, `hooks/` scaffolded; (h) connector-probe + graceful-degradation matrix codified (Coda MCP / Google Workspace MCP / Miro path / Pipefy / Wrike / Ziflow auth + native-AI ingestion paths); (i) all cosmetic CONCERNS items cleaned per Appendix B. |
| Depends on | — (root phase) |
| Addresses | FOUND-01, FOUND-02, FOUND-03, FOUND-04, FOUND-05, FOUND-06, FOUND-07 |
| Avoids pitfalls | CRIT-6 (frontmatter migration corrupts in-flight builds), MIN-5 (stage-numbering orphans), MOD-16 (hard-rules duplicate-and-edit). MIN-6 (email mismatch) closed/withdrawn under UAT-3.1 — email is intentional per dYdX Digital approval. |
...
| Research-blocked | ⚠ Pipefy GraphQL pagination cursor field names + Pipefy/Wrike 2026 rate-limit currency + Ziflow read-after-create consistency window unverified — see Appendix C. Native-AI API ingestion items (Wrike AI Studio / Ziflow ReviewAI) WITHDRAWN under UAT-6.1.<br><br>**Inline markers (per D-27 carried):**<br>• `[OPEN: Phase 4 — Pipefy GraphQL pagination cursor field names need verification against current 2026 schema per OPEN-01]`<br>• ~~`[OPEN: Phase 4 — Wrike AI Studio knowledge-ingestion API not externally verified per OPEN-01 — Phase 7 owner per CHANGE-04]`~~ — **WITHDRAWN UAT-6.1**<br>• ~~`[OPEN: Phase 4 — Ziflow ReviewAI knowledge-ingestion API not externally verified per OPEN-01 — Phase 7 owner per CHANGE-04]`~~ — **WITHDRAWN UAT-6.1**<br>• `[OPEN: Phase 4 — Ziflow read-after-create consistency window unverified per OPEN-01; Phase 2 owner per CHANGE-04. Conservative default in helper: 30 second poll with 2s interval.]` |
...
> **Why this phase here.** Per-platform path; depends on platform skills (Phase 2) and approved doc fragments (Phase 6). REVISED under UAT-6.1 (2026-05-10): scope reduced from "API push + paste fallback" to "paste bundle + upload audit log." API ingestion paths (Pipefy AI KB / Wrike AI Studio / Ziflow ReviewAI knowledge-ingestion APIs) were the SINGLE BIGGEST research blocker and have been removed from scope entirely. Tool produces correctly-shaped upload instructions; humans upload manually via each platform's UI. Phase 7 is now UNBLOCKED. *(per `.planning/research/SUMMARY.md` § "Phase 7 — Native-AI Knowledge Push (Stage 10)" + § "Phase Ordering Rationale": "Phase 6 before Phase 7 because Stage 10 ingests approved doc fragments from Stage 9.")*

| Attribute | Detail |
|---|---|
| Deliverables | (a) `push-native-ai-knowledge/` skill (Stage 10, NEW; REVISED under UAT-6.1); (b) reads `04a` + approved doc fragments from Stage 9 + per-platform `references/native-ai-inventory.md` (kept as human-readable reference for what to upload); (c) branches on `native_ai_path: paste \| none` only (`api` branch REMOVED under UAT-6.1); (d) refuses to emit upload instructions if `doc_published_at < last_diff_review_at` (CRIT-8 fix carried via "refuse to instruct"); (e) per-client target ID in `00_HUB.md` `Pipefy AI:` / `Wrike AI:` / `Ziflow AI:` blocks; (f) refuses to emit upload instructions if target mismatches `client:` frontmatter (MIN-4 fix); (g) `doc_version: <semver>` + `uploaded_at: <ISO>` + `paste_confirmed: <ISO>` per fragment in the upload audit log; (h) re-run idempotency — already-uploaded fragments skipped on re-run. |
| Depends on | Phase 2 (platform-pipefy/-wrike/-ziflow `native-ai-inventory.md` references — kept as human-readable reference under UAT-6.1), Phase 6 (approved doc fragments from Stage 9) |
| Addresses | STG10-01, STG10-02, STG10-03 (REVISED scope under UAT-6.1; previous "BLOCKED BY OPEN-01" framing OBSOLETE — Q01/Q02/Q03 withdrawn) |
| Avoids pitfalls | CRIT-8 (knowledge-ingestion races doc publishing — refuses to emit upload instructions if `doc_published_at < last_diff_review_at`), MIN-3 (knowledge-versioning gap — `doc_version` + `uploaded_at` per fragment), MIN-4 (multi-tenant knowledge leak — refuses to emit upload instructions if target mismatches `client:`) |
| Skills introduced/modified | 1 NEW (`push-native-ai-knowledge`, Stage 10). Appendix A row 15. |
| Research-blocked | — UNBLOCKED under UAT-6.1 (2026-05-10). Q01/Q02/Q03 (Pipefy AI KB / Wrike AI Studio / Ziflow ReviewAI knowledge-ingestion APIs) → `Status: closed` under UAT-6.1; native-AI API ingestion is OUT OF SCOPE entirely. Tool produces docs; humans upload manually via platform UIs. Re-evaluation of API ingestion is a post-v2.6 milestone gated on first-real-client-engagement-practice-run completion. |
...
| none | `push-native-ai-knowledge/` | NEW | Stage 10 — paste bundle + upload audit log (REVISED under UAT-6.1, 2026-05-10). Branches on `native_ai_path: paste \| none` only; `api` branch REMOVED under UAT-6.1. Tool produces upload instructions; humans upload manually via platform UIs.<br>Refuses to emit upload instructions if `doc_published_at < last_diff_review_at` (CRIT-8 fix carried via "refuse to instruct")<br>Refuses to emit upload instructions if target mismatches `client:` frontmatter (MIN-4 fix)<br>`uploaded_at:` per fragment for re-run idempotency | Phase 7 (v2.5) — UNBLOCKED under UAT-6.1 | DESIGN-26 |
...
#### B.5 Owner-email — INTENTIONAL (NOT a fix; UAT-3.1 reclassification 2026-05-10)

- **Citation:** `.claude-plugin/marketplace.json:5` (`owner.email`) and `dydx-delivery/.claude-plugin/plugin.json:7` (`author.email`) both list `jasonmichaelb@gmail.com`.
- **Context:** Project owner's private email is intentionally listed on the plugin manifests — explicitly approved by dYdX Digital. NOT a defect; NOT a deferred fix.
- **Severity:** **[NOT A FINDING]** — UAT-3.1 reclassification (2026-05-10).
- **Action:** No fix. Removed from FOUND-04 v2.1 Foundations scope. **Cross-ref:** MIN-6 PITFALL marked **closed/withdrawn** under UAT-3.1.
...
|---|---|---|
| **Phase 1 (v2.1)** — Foundations + Connector Verification | - **Coda MCP version pin** — install latest, capture exact version. Per `.planning/research/STACK.md` "Stack open questions" #1.<br>- **Google Workspace MCP choice** — `taylorwilsdon/google_workspace_mcp` recommended per RESEARCH.md (MEDIUM confidence, community); avoid splitting Drive/Gmail/Calendar into separate servers. Pin specific version; never `@latest`. Per STACK.md #2.<br>- **Miro MCP existence** — no canonical Miro MCP found in Context7. Stage 1 kickoff branches: API ingest if available, image paste otherwise. Do NOT assume "render whole board" endpoint exists. Per STACK.md context.<br>- **Wrike rate-limit currency (2026)** — 200 req/min from v0.3.0 `safety-rules.md` not re-verified. Per STACK.md #4 + DESIGN.md Appendix E bullet 5.<br>- **Ziflow auth header + base URL** — verified only at `dydx-delivery/skills/generate-technical-spec/references/technical-spec-template.md:113`. Per STACK.md #5. | `/gsd-research-phase 1` BEFORE v2.1 plan-phase locks the connector-probe plan. Probe verifies: Coda MCP version pin; Google Workspace MCP choice; Miro MCP existence/path; Wrike rate-limit currency; Ziflow auth header + base URL. Outcome: per-tenant graceful-degradation matrix per the PITFALLS connector-fallback table. |
| ~~**Phase 7 (v2.5)**~~ | **REMOVED under UAT-6.1 (2026-05-10).** The 3 historic BLOCKERs (Pipefy AI KB content-upload endpoint / Wrike AI Studio knowledge-ingestion API / Ziflow ReviewAI knowledge-ingestion API) are WITHDRAWN — native-AI API ingestion is OUT OF SCOPE entirely. Phase 7 is no longer research-blocked. Tool produces docs; humans upload manually via platform UIs. Re-evaluation of API ingestion is a post-v2.6 milestone gated on first-real-client-engagement-practice-run completion. | — Phase 7 ships in v2.5 alongside Phase 6 with simplified scope (paste bundle + upload audit log). No `/gsd-research-phase 7` invocation needed. |
...
This list enumerates every inline OPEN-Phase-4 marker in CHANGELIST.md, in document order, as the canonical handoff to Phase 4 OPEN-QUESTIONS.md. Each item names the source section + recommended owning phase per CHANGE-04 (or `owner: TBD` if not yet decided — never silently omitted). Phase 4 register builds against this list mechanically.

**Cardinality note (Phase 3 deviation from DESIGN.md baseline).** DESIGN.md §"Deferred to Phase 4 OPEN-QUESTIONS" carries 8 baseline bullets. Phase 3 authoring discovered one net-new deferral per D-27 ("New deferrals discovered during Phase 3 authoring add a row here AND inline at point of use") — bullet 9 below: `plugin self-test scope per OPEN-07` placed in the Phase 9 mini-table Deliverables cell by Phase 3 Wave 3 plan (pulled forward from REQUIREMENTS.md OPEN-07). The structural-check assertion is `>= 8` (a floor, not equality) and accommodates the count of 9 cleanly. Total inline markers = 9; total Appendix E bullets = 9; reconciliation algorithm walks 9 → 9.
...
- ~~`[OPEN: Phase 4 — Pipefy AI KB content-upload endpoint not externally verified per OPEN-01 — Phase 7 owner per CHANGE-04]`~~ — **WITHDRAWN under UAT-6.1 (2026-05-10).** Native-AI API ingestion is OUT OF SCOPE entirely; Q01 status `closed`. Marker preserved in audit trail; no longer counted as active deferral.
- **`[OPEN: Phase 4 — plugin self-test scope per OPEN-07]`** — source: §Phase 9: Surfaces (v2.6) → mini-table Deliverables cell (item g — optional plugin self-tests at `dydx-delivery/tests/` per DESIGN-04 + D-24) — owner: Phase 9 per CHANGE-04 (scope decision pulled forward from REQUIREMENTS.md OPEN-07; net-new Phase 3 deferral per D-27)

**Verification (run by Phase 4 / final reviewer).** The reconciliation algorithm in Phase 3 Wave 7 synthesis verifies (a) inline-marker text count outside this section matches list-bullet count; (b) `diff` of normalised marker texts is empty; (c) every bullet carries `owner:` (Phase or TBD). Mismatch on any condition halts Phase 4 register build. Algorithm precedent: Phase 2 02-10 PLAN.md Task 2 (cross-AI HIGH #3 fix carried). New deferrals discovered after Phase 3 approval add a bullet here AND inline at point of use; reconciliation re-runs.
