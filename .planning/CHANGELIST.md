...
| **Phase 7 (v2.5)** — Native-AI knowledge push **[BLOCKED]** | - **Pipefy AI KB content-upload endpoint** — not visible in Pipefy Help Center / Pipefy Developers GraphQL reference (Agent CRUD verified; KB upload NOT visible). Per RESEARCH.md SUMMARY.md "Sources" §Tertiary + STACK.md #6.<br>- **Wrike AI Studio knowledge-ingestion API** — could not be externally verified. Wrike MCP server documents 16 tools (task/folder/project CRUD, search, custom fields, workflows, contacts, approvals); knowledge ingestion not in that list. Per STACK.md #6.<br>- **Ziflow ReviewAI knowledge-ingestion API** — Ziflow ReviewAI page documents Checklists Public Preview + Change Verification + Brand Standards "Coming Soon"; knowledge-ingestion API not visible. Per STACK.md #6.<br><br>**OPEN-01 cross-reference:** OPEN-01 (per `.planning/REQUIREMENTS.md`) catalogues every "couldn't verify" item from research with owning phase + verification owner. The 3 native-AI APIs above are the SINGLE BIGGEST unknown.<br><br>**OPEN-01 contingent fallback (verbatim per D-37 — MANDATORY per cross-AI C3):** "If OPEN-01 (native-AI ingestion paths per platform) remains unresolved at v2.5 kickoff, split P6 → v2.5 (Documentation only) and P7 → v2.6 (Native-AI when unblocked); slide P8/P9 → v2.7. Phase 7 is the only blocked-by-OPEN phase; isolating it preserves cadence on the rest." (Also present in Phase 7 mini-table Research-blocked cell per Phase 3 Wave 3 plan — belt-and-braces.) | `/gsd-research-phase 7` **MANDATORY** before v2.5 plan-phase locks Phase 7. **HARD BLOCKER per OPEN-01.** Probe verifies: Pipefy AI KB content-upload endpoint; Wrike AI Studio knowledge-ingestion API; Ziflow ReviewAI knowledge-ingestion API. Outcome: `native_ai_path: api \| paste \| none` decision per platform. If any API remains unverified, fall back to copy-paste path (default per DESIGN-26). |
