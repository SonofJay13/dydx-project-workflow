...
| **Config file** | none |
| **Quick run command** | `bash .planning/phases/01-audit/scripts/audit-structure-check.sh` (created in Wave 0; runs grep/file-exists assertions in <5s) |
...
- [ ] `.planning/phases/01-audit/scripts/audit-structure-check.sh` — bash script that:
  - asserts `.planning/AUDIT.md` exists
  - asserts every `## AUDIT-0N: <Name>` heading is present (8 sections)
  - asserts the executive summary table exists (`## Executive Summary` or first table after preamble)
  - asserts `## Appendix B: CONCERNS.md → AUDIT.md trace` exists with at least 14 rows (one per CONCERNS.md section)
  - asserts AUDIT-07 cosmetic-fix bullets each contain the literal string "scheduled for v2.1 build"
  - asserts AUDIT-06 recommends `2.0.0` as synced target version
  - exits 0 on all-pass, 1 on any failure with a clear message
...
| "What's missing for v2" callouts stay observation-led (no design moves proposed) | D-13, AUDIT-01 | The audit must describe what's missing and point at the DESIGN-* requirement; it must NOT propose how the v2 design closes the gap. Reviewer judges tone. | Read each per-skill "What's missing for v2" subsection. Confirm: (a) it names the gap, (b) it points at a DESIGN-NN requirement in REQUIREMENTS.md, (c) it does NOT prescribe how DESIGN-NN should solve the gap. |
| AUDIT-07 cosmetic-fix bullets carry the v2.1 constraint per-bullet (not just at section header) | D-16, AUDIT-07 | The structural check asserts the literal string is present per bullet; the reviewer confirms no bullet was structured to defeat that check (e.g., wrapping all bullets in a single "scheduled for v2.1" paragraph above and not in each bullet). | Read AUDIT-07 section. For every cosmetic-fix bullet, confirm the bullet itself contains "scheduled for v2.1 build, NOT this milestone" (or equivalent phrasing). |
